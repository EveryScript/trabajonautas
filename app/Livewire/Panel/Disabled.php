        ...
        return <<<'HTML'
            <section class="flex items-center justify-center min-h-screen bg-gradient-to-br">
                <div class="text-center animate-fadeIn">
                    ...
                    <p class="text-sm text-tbn-dark mt-3 mb-6">El administrador de Trabajonautas ha deshabilitado su acceso al sistema.
                    Comuníquese con él para recuperar su cuenta.</p>
                    <form method="POST" action="{{ route('logout') }}" x-data>
                        @csrf
                            <x-button-link class="bg-tbn-primary" href="{{ route('logout') }}" @click.prevent="$root.submit();">Cerrar sesión</x-button-link>
                    </form>
                </div>
            </section>
        HTML;
