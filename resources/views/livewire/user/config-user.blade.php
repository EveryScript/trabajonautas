<section>
    ...
    <form class="max-w-2xl" wire:submit="{{ $id ? 'update' : 'save' }}">
        <div class="mb-4">
            ...
            <x-input wire:model="user.name" id="name" type="text" class="block w-full mt-1"
                placeholder="Sergio Rodriguez" />
            <x-input-error for="user.name" class="mt-2" />
        </div>
        <div class="mb-4">
            <x-label for="email" value="{{ __('Email') }}" />
            <x-input wire:model="user.email" id="email" type="email" class="block w-full mt-1"
                placeholder="example@email.com" />
            <x-input-error for="user.email" class="mt-2" />
        </div>
        <div class="mb-4">
            <x-label for="password" value="{{ $id ? __('Nueva contraseña') : __('Contraseña') }}" />
            <x-input-password wire:model="user.password" id="password" type="password" class="block w-full mt-1" />
        ...
        <div class="mb-4">
            <x-button type="submit">{{ $id ? 'Actualizar usuario' : 'Crear usuario' }}</x-button>
