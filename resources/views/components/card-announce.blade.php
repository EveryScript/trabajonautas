@props([
    'logo_flag' => true,
    'logo_url' => '#',
    'title' => 'Mi título',
    'pro' => false,
    'locations' => 'Mis ubicaciones',
    'created_at' => now(),
])

<article
    class="w-full h-full relative flex md:flex-row flex-col justify-start gap-4 md:gap-6 text-start
            bg-white dark:bg-tbn-dark dark:text-tbn-light shadow-md px-6 py-5 rounded-lg border border-gray-300 dark:border-tbn-secondary hover:border-tbn-primary transition-colors duration-150 cursor-pointer">
    <img class="flex-shrink-0 rounded-lg w-16 h-16 object-cover object-center {{ $logo_flag ? '' : 'hidden' }}"
        src="{{ !empty($logo_url) ? asset('storage/' . $logo_url) : asset('storage/empresas/tbn-new-default.webp') }}">
    <div class="flex-1 text-sm">
        <p class="absolute top-4 right-6 {{ $pro ? '' : 'hidden' }}">
            <i class="fas fa-crown text-sm text-tbn-primary"></i>
        </p>
        <p class="text-xs font-normal text-tbn-dark dark:text-tbn-light">
            <span class="pr-5">Publicado {{ Carbon\Carbon::parse($created_at)->diffForHumans() }}</span>
        </p>
        <h2 class="font-bold text-lg leading-6 my-1 dark:text-white">{{ $title }}</h2>
        <div class="w-full grid grid-cols-2 mt-1">
            @if (isset($company))
                <h3 class="text-tbn-primary font-bold">{{ $company }}</h3>
            @else
                <p class="text-tbn-dark dark:text-tbn-light text-sm">(sin empresa)</p>
            @endif
            <p class="relative">
                <i class="inline fas fa-map-marker-alt text-tbn-primary mr-2"></i>{{ $locations }}
            </p>
        </div>
    </div>
</article>
