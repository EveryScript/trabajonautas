...
<div x-data="{ show: @entangle($attributes->wire('model')) }" x-on:close.stop="show = false" x-on:keydown.escape.window="show = false" x-show="show"
    id="{{ $id }}" class="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black jetstream-modal bg-opacity-60"
    ...
    <div x-show="show" class="fixed inset-0 transition-all transform" x-on:click="show = false"
        ...
        <div class="absolute inset-0 opacity-75 bg-tbn-light dark:bg-tbn-dark"></div>
    </div>

    <div x-show="show"
        class="mb-6 bg-white dark:bg-neutral-900 rounded-lg overflow-hidden shadow-xl transform transition-all sm:w-full {{ $maxWidth }} sm:mx-auto"
