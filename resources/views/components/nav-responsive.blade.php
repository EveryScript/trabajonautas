<header class="bg-white dark:bg-tbn-dark py-4">
    ...
    <div x-data="{ open: false, dropdown: false }" @click.away="open = false, dropdown = false"
        ...
        @role([env('USER_ROLE'), env('ADMIN_ROLE')])
            <a href="{{ route('welcome') }}" wire:navigate>
                <img class="inline-block dark:hidden max-w-12" src="{{ asset('storage/img/tbn-new-isologo.webp') }}" alt="tbn-logo">
                <img class="hidden dark:inline-block max-w-12" src="{{ asset('storage/img/tbn-white-isologo.webp') }}" alt="tbn-logo">
            </a>
        @endrole
        <div class="flex items-center md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse relative">
            @auth
                ...
                <!-- Dropdown menu -->
                <div x-show="open"
                    class="absolute top-6 right-0 z-50 my-4 text-base list-none bg-white dark:bg-tbn-dark border border-tbn-light dark:border-tbn-secondary divide-y divide-gray-100 dark:divide-tbn-light rounded-lg shadow"
                    id="user-dropdown">
                    <div class="px-4 py-3">
                        <span class="block text-sm font-medium text-tbn-secondary dark:text-white">{{ Auth::user()->name }}</span>
                        <span class="block text-sm text-tbn-secondary dark:text-tbn-light truncate">{{ Auth::user()->email }}</span>
                    </div>
                        ...
                        <li>
                            ...
                            <form method="POST" action="{{ route('logout') }}" x-data>
                                ...
                                <a href="{{ route('logout') }}" @click.prevent="$root.submit();"
                                    class="block px-4 py-2 text-sm text-tbn-primary hover:bg-tbn-light dark:hover:bg-neutral-900">
                                    Cerrar sesión</a>
                            </form>
                        </li>
                    </ul>
        ...
        @auth
            <div x-bind:class="dropdown ? '' : 'hidden'" 
                class="bg-white dark:bg-tbn-dark items-center justify-between w-full md:flex md:w-auto md:order-1" id="navbar-user">
                <ul x-transition:enter.duration.300ms x-transition:leave.duration.300ms
                    class="flex flex-col text-tbn-secondary dark:text-tbn-light font-medium text-sm md:p-0 mt-4 rounded-lg md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0">
                    @role('USER|ADMIN')
